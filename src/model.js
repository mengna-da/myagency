	...
	init() {
		...
		this.loader.load(this.file, (gltf) => {
			...
			if (this.animations) {
				...
				// Store animations in the mesh's userData for access
				gltf.scene.userData.animations = gltf.animations
				// Store animation names in the global availableAnimations array
				if (gltf.animations && gltf.animations.length > 0) {
					gltf.animations.forEach((clip) => {
						if (clip && clip.name) {
							window.availableAnimations.push(clip.name)
						}
					})
				}
			...
			//we're taking the values we passed in and setting the values of our 3d model to said parameters
